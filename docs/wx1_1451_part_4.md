...
### 5.1 Download a Webex token and create an Environment in CE-Deploy
...
5.1.1 Open CE-deploy and select the Download Webex Token button in the manage token section

![5-1-1.png](assets%2Fwx1_1451_part_4%2F5-1-1.png)

5.1.2 Sign in to Webex and accept the integration permissions when prompted. This will load 
...
5.1.3 In the deployment panel select "Environment".

![5-1-3.png](assets%2Fwx1_1451_part_4%2F5-1-3.png)

5.1.4 In the environment name section name your environment "EnvLab" and click "save Environment".

5.1.5 To load our new environment, use the drop down in the Environment loading section and select your 
new Environment  and select "Load Environment".

![5-1-5.png](assets%2Fwx1_1451_part_4%2F5-1-5.png)

5.1.6 You should now see your environment name populate the Environment Loaded text under manage Token.

![5-1-6.png](assets%2Fwx1_1451_part_4%2F5-1-6.png)

5.1.7 To test your token click the "Test Token" button. You should see the response below:

![5-1-7.png](assets%2Fwx1_1451_part_4%2F5-1-7.png)

Congrats, your token is now downloaded and ready to roll.

### 5.2 Deploy xAPI commands via CE-Deploy
...
5.2.1 Open Ce-deploy and load the environment your created in the last lab.

5.2.2 	2. Select xAPI from the deployment section. In the command text box enter:
```text
...
5.2.3 Under the deployment options drop down select **Org Id**, this will populate the lab org ID. 
...
5.2.4 Select the **video devices only** filter checkbox. This will limit running the command to only 
the xAPI capable device in the organization and no other devices in the lab, an example being a phone.

5.2.5 Select **"Start Deployment"**

5.2.6 Ensure **Run Deployment Now** is selected and select **Next**.

5.2.7 If any devices are offline you may receive a prompt letting your know that some of the devices are offline. 
If this is case just select **"Yes"**.

5.2.8 The message console will now let you see the deployment in progress and all output from the devices. 
In this case what is returned is the system time running on the endpoints.

![5-2-8.png](assets%2Fwx1_1451_part_4%2F5-2-8.png)

5.2.9 This concludes this section. While this is a simple use case, any xAPI command can be run in this fashion 
including multiline commands that contain a body section.

### 5.3 Deploy Banding via CE-Deploy
...
5.3.1 Open CE-Deploy and load the environment your created in a previous lab.

5.3.2 In this lab we will be load a background and logo on to our lab device but to help us do that first 
...
5.3.3 Add a tag using your pod ID

![5-3-3.png](assets%2Fwx1_1451_part_4%2F5-3-3.png)

5.3.4 The widespread adoption of QR codes has introduced unique advantages to branding. While 
...
### 5.4 Deploy Macros via CE-Deploy
...
5.4.1 Open CE-Deploy and select Design>Macro Editor
...
### 5.5 Deploy Extensions via CE-Deploy
...
### 5.6 Deploy a local user admin using CE-Deploy
...
### 5.7 Deploy xAPI command via the shell using CE-Deploy
...
### 5.8 Perform a device backup using CE-Deploy
