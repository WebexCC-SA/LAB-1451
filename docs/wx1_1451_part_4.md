...
4.10.13 You just deployed a macro to your device and activated it but we still need one more step 
to ensure that it is in fact running. Unlike the device web admin portal which will restart the macro engine
automatically everytime a change is made to a macro the API's are not as polite. So in the next lab
